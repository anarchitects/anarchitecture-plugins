...
| `packages/governance/src/extensions/host.ts`                         | Registers already-discovered extensions, validates extension definitions, and applies extension contributions deterministically.                                                                          | Shared extension runtime utility; contracts in core                                         | `extension-host`, `core-candidate`                                                                              | Nx-specific discovery and `nx.json` loading should stay outside this file.                                                                                                                                                                                                                         |
| `packages/governance/src/nx-host/extensions/*`                       | Loads governance extension config from `nx.json`, probes legacy Nx plugins, and translates Nx plugin configuration into extension load requests.                                                          | `@anarchitects/nx-governance` Nx host                                                       | `nx-host`                                                                                                       | Clear Nx-host responsibility. Keeps Core-facing extension runtime concepts free of `@nx/devkit` and `nx.json` loading.                                                                                                                                                                             |
...
2. `nx-host/extensions/host.ts` reads `nx.json`, discovers extensions from `nx.json.plugins`, and probes `<plugin>/governance-extension` for each non-local plugin.
...
- `nx-host/extensions/host.ts` reads `nx.json` and probes `<plugin>/governance-extension` for entries from `nx.json.plugins`.
