...
import showMigrations from './executor.js';
...
describe('migration-show executor', () => {
  ...
  beforeEach(() => {
    ...
    tempDir = mkdtempSync(join(tmpdir(), 'nx-typeorm-migration-show-'));
  ...
  it('uses inferred migration datasource when omitted', async () => {
    ...
    const result = await showMigrations({}, context);
